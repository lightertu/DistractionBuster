...
def drawgraph(x_label,y,file,text_data):
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x, y, width, color='blue',align='center')
    ax.set_ylabel('Weights')
    ax.set_title('Topic Distribution of: ' + file.replace(".xml",""))
    
    plt.tight_layout(pad=3, w_pad=0.1, h_pad=0.1)
    #plt.subplot(100,200,100)
    ax.title.set_position([.5, 1.2])
    #ax.set_xticks(x + width / 2)
    #ax.set_xticklabels(category_list)
    #ax.text(3, 8, text_data, style='italic',bbox={'facecolor':'green', 'alpha':0.5, 'pad':10})
    ...
    def autolabel(rects):
        ...
        for rect in rects:
            ...
            #ax.text(rect.get_x() + rect.get_width()/2.5, 1.05*height,'%d'% int(height), va='bottom')
            ax.text(rect.get_x() + rect.get_width()/2.5, 1.05*height,round(height,2))
...
for path, subdirs, files in os.walk(test_folder):
    for file in files:
        if file.endswith('.xml'):
            ...
            drawgraph(list(centroids_dict.keys()),graph_data,root_folder+model_name+'/'+file,text_data)
